...
*Revision 2026-05-23 — fix of three permission regressions of the same class (an `isValidX(incoming())` validator used as a global AND on an `allow update` while containing a `data.<field> == request.auth.uid` check, which blocks every legitimate cross-user write):*
- ***Cross-user approval / role-change rejected.** The `isValidUser` validator enforced `data.uid == request.auth.uid` as a global AND over **every** `users.update` branch. When an Admin/Root wrote another user's doc (approval `pending → approved`, promotion, role change), `incoming().uid` was the target's uid, not the caller's: the identity check failed before the admin/root branch was ever evaluated, **freezing every new sign-up in `pending`** with no way to enable it. Re-based to `data.uid == userId` (the `uid` field must equal the doc id) — correct for both self-writes (create already enforces `userId == request.auth.uid`) and admin→other-user writes. The §2 branches were correct; the global guard was blocking them.*
- ***"Like Forger" hardened (Sporca #31).** `isValidPost` (which enforces `authorId == request.auth.uid`) is no longer a global AND of `posts.update`: a like is a partial update that leaves `authorId` untouched, so `incoming().authorId` stayed the author's uid and the check **blocked likes from anyone but the author**. `isValidPost` now gates only the author-edit branch; the like/unlike and comment-bump branches self-validate (`affectedKeys().hasOnly` + set diff pinned to the caller's uid + `likedBy ≤ 5000` cap). The redundant leading `isAdminOrRoot() || …` term — always true under the top gate — was also removed; it let an author-admin bypass the anti-forge logic (double-like).*
- ***Event RSVP rejected.** Same pattern on `events.update`: `isValidEvent` (which enforces `authorId == request.auth.uid`) was a global AND. RSVP (`EventDetailModal.handleRSVP`) is a partial update of only the `attendees` field that leaves `authorId` untouched → **only the event author could RSVP**; everyone else was rejected. `isValidEvent` now gates only the author-edit branch; RSVP is a dedicated branch pinned to the caller's **own entry** in the `attendees` map (`attendees.diff(...).affectedKeys().hasOnly([request.auth.uid])`, anti "attendee forger").*
- *Note: `expenses.update` carries the same validator with `paidBy == request.auth.uid`, but the client never updates an expense (only create/read/delete) → dormant footgun, left unchanged for lack of a use case.*
- *All three fixes covered by regression tests in `firestore.rules.test.ts` (describe "Admin/Root user management (cross-user users.update)", "Sporca #31 — Like Forger", "Events update (RSVP cross-user + author edit)").*
...
- **Mapped Updates:** Specific actions, on distinct `posts.update` branches. **(a) Author edit:** only the author (`request.auth.uid == existing().authorId`, gated by `isValidPost`) acts on `caption`, `visibilityStatus`, `visibilityTime`, `showInCinematografo`, or `location`. **(b) Like:** any enabled user (the collection is admin/root-only) can like/unlike — including on others' posts, as is natural for hearts — with double validation via array tracking (`likedBy`) mathematically tying the update to paired logics on `likesCount` and **pinning the added/removed element to the caller's uid** (set diff), preventing loops, count falsifications, infinite upvotes and injection of someone else's uid (Sporca #31). **(c) Comment bump:** isolated `commentsCount ±1`. None of these branches is admin/root-bypassable: the logic applies to everyone (see revision note 2026-05-23).
