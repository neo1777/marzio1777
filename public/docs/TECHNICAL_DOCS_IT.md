...
interface AudioSession {
  ...
  // Flag one-way (false → true, mai revertibile via rule). Settato nel batch
  // di chiusura sessione quando totalDurationMs > 30 min, per accreditare il
  // +10 long-session bonus al DJ una sola volta. Anti double-spend.
...
interface QueueItem {
  ...
  // Audit P2P (server-stamped via serverTimestamp, non Date.now())
  transferStartedAt?: Timestamp;
  transferCompletedAt?: Timestamp;
  transferFailureReason?: string;

  pointsAwarded?: number;            // assegnati a 'played' (= round(2 × eventMultiplier))

  // Snapshot della formula bonus al create-time, validato dalla rule
  // contro `effectiveMaxQueued(sessionId)` per chiudere il vettore
  // "Sporca #24 Queue Stuffer" (90% — il count effettivo dei doc attivi
  // resta CF Fase 2: il DSL non può contare documenti).
...
3. **Integrità Esplicita:**
   ...
   - Per `quizRounds/answers`: rule garantiscono `selectedIndex ∈ [0..3]`, `userId == request.auth.uid`, `submittedAt < endsAt`. Per la `update` (assegnazione `pointsAwarded`) il **path preferito è il self-claim post-reveal** (`answerUserId == request.auth.uid && isEventParticipant`), con override host-triade per Root recovery: la rule re-deriva la correttezza da `correctIndex` (pubblico post-reveal) e cap-pa `pointsAwarded ≤ photoQuizConfig.maxPointsPerRound × pointsMultiplier` (default `100 × 1 = 100`). Vedi §4.8 per il flow client.
   ...
   - Per `game_events/{}/leaderboard.write` e `game_events/{}/participants.delete`: ownership stretto (`userId == request.auth.uid` oppure organizer/Root). Pre-B7 entrambe le rule erano "any approved user", consentendo a un partecipante di azzerare il leaderboard di un avversario o di kickarlo dalla lista partecipanti.
   - Per `users.update`: increment owner-side di `points` ammesso fino a **+1000/transaction** (era +50 pre-B7, sottodimensionato rispetto al range `pointsMultiplier ∈ [0.5, 5.0]`). Il cap blocca comunque il forging catastrofico (`points: 9999999`) mentre lascia spazio agli accrediti legittimi di cattura/quiz/audio scalati per il moltiplicatore evento. Anti-cheat reale vive nelle rule dell'azione che assegna i punti.
   - Per `audio_sessions/{}/queue.update`: `affectedKeys().hasOnly(['status', 'position', 'transferStartedAt', 'transferCompletedAt', 'transferFailureReason', 'pointsAwarded'])` + immutabilità esplicita di `proposedBy`/`localTrackId`/`trackTitle`/`trackArtist`/`trackDurationMs` (defense-in-depth). Cap `pointsAwarded ∈ [0, 50]`. Questo chiude al 100% le Sporche #25/#26 "Theme Hijacker" (B7).
   - Per `audio_sessions` e sub-collections: rule analoghe come da §10. La macchina a stati `validQueueStatusTransition` ora forza il passaggio `queued → transferring` (no shortcut diretto a `ready`/`failed`) come da spec.
...
- Ogni partecipante crea il proprio `answers/{userId}` (rule: solo prima di `endsAt`, una sola volta) con `pointsAwarded: 0` placeholder.
- Allo scadere del timer, l'host chiama `revealRound(eventId, roundId)`: legge il `correctIndex` dal `secret/` e fa una singola update sul doc parent (`status: 'revealed'`, `revealedAt: serverTimestamp()`, `correctIndex`). La rule sblocca lettura di `correctIndex` e lettura aggregata delle `answers/*`.
- **Scoring owner-side post-reveal** (`claimMyAnswerPoints` in `usePhotoQuiz.ts`): ogni partecipante, via `useEffect` su `round.status === 'revealed'`, esegue una `runTransaction` locale che:
  - rilegge il proprio `answers/{me}` e calcola `pointsAwarded` con `calculateQuizPoints(scoringMode, isCorrect, elapsedMs, windowMs, maxPointsPerRound) × eventMultiplier`
  - aggiorna `answers/{me}.pointsAwarded`
  - se > 0, fa `set merge` su `leaderboard/{me}` con `increment(pts)` e `update` su `users/{me}.points` con `increment(pts)`
  - idempotenza via `localStorage[marzio1777:quiz-claimed:{roundId}:{uid}]` — il claim è single-shot per round-utente
- La rule su `answers.update` accetta sia il self-claim (`answerUserId == request.auth.uid && isEventParticipant`) sia l'override host-triade, e in entrambi i casi: richiede `revealedAt != null`, valida `pointsAwarded ∈ [0, photoQuizConfig.maxPointsPerRound × pointsMultiplier]`, impone `selectedIndex == correctIndex` quando `pointsAwarded > 0`, e cap-pa l'increment su `users/{me}.points` a +1000/transaction (vedi §3).
- **Razionale del split** (B7, Maggio 2026): la versione precedente (`evaluateRoundAnswers`) faceva l'host eseguire `tx.update(users/{otherUid}, { points: increment(...) })` per ogni partecipante con risposta corretta. La rule `users.update` consente legittimamente solo all'owner di mutare il proprio `points` doc, quindi quel flusso era respinto in produzione. Il self-claim ribalta la pipeline: l'host dichiara la verità (correctIndex pubblico), ogni client la verifica e si auto-accredita. Niente bottleneck host, niente race su sessioni concorrenti, anti-cheat invariato (la rule re-deriva tutto dal correctIndex pubblico).
...
**Vincoli su `queue.update` (B7 hardening, Maggio 2026)**:
- Solo `isSessionDJ(sessionId)` può chiamare update
...
- Defense-in-depth ridondante: `proposedBy`/`localTrackId`/`trackTitle`/`trackArtist`/`trackDurationMs` confrontati esplicitamente con `existing()` e respinti se cambiati
- `validQueueStatusTransition(oldStatus, newStatus)` forza il flow `queued → transferring → ready/failed → playing/skipped → played` (no shortcut diretto da `queued` a `ready`)
- `pointsAwarded ∈ [0, 50]` quando modificato — coerente con `BASE_TRACK_POINTS × eventMultiplier_max = 2 × 5 = 10` con margine
- Risultato: chiusura al 100% delle Sporche #25 e #26 (Theme Hijacker, anche nella variante DJ)
...
### Route aggiunte
Tutto sotto `/dashboard/*` (BrowserRouter `basename={import.meta.env.BASE_URL}`,
`<Layout />` come parent route protetto).

```
# Campo dei Giochi (route piatte)
/dashboard/giochi                          → IlCampoDeiGiochi (hub eventi attivi/in arrivo)
/dashboard/giochi/nuovo                    → GameCreator (wizard, Admin/Root)
/dashboard/giochi/:eventId/lobby           → GameLobby
/dashboard/giochi/:eventId/play            → GamePlayRouter (smista A/B su event.type)
/dashboard/giochi/:eventId/results         → GameResults

# L'Ainulindalë (router nidificato in IlAinulindale.tsx)
/dashboard/ainulindale                     → PersonalLibrary (Biblioteca, default)
...
/dashboard/ainulindale/sessioni/:id        → AudioSessionWrapper
                                             (smista DJ vs Listener su djId === user.uid)
/dashboard/ainulindale/sessioni/:id/dj     → AudioSessionDJ (accesso diretto)
```
...
- **L'Ainulindalë:** `useLocalLibrary`, `useAudioPlayer` (Walkman state-level: queue, shuffle, repeat, MediaSession), `useAudioEngineRaw` (DJ flow: engine, playBlob, getCurrentTime, getDuration, isPlaying — separato da `useAudioPlayer` per pulizia), `useAudioSession`, `useAudioQueue`, `useSessionParticipants`, `useWebRTCTransfer` (esporta `useWebRTCTransferDJ` e `useWebRTCTransferProposer`), `useMediaSession`, `useStorageQuota`
- **Trasversali:** `useRBAC` (wrapper di `useAuth` con flag derivati `isRoot`/`isAdmin`/`isGuest`/`isAdminOrRoot`/`isApproved`/`isPending`); `useWakeLock(active)` (acquisisce/rilascia `screen` wake-lock con guard su `permissionsPolicy`/`featurePolicy` e re-acquire automatico su `visibilitychange === 'visible'`).
...
- **Gagliardetti audio (Fase 2 chiusa, Maggio 2026):** *Il Cantore* (50 proposte), *Il Sub-Creatore* (25 proposte played), *Il Conduttore* (5 sessioni come DJ), *Il Maestro del Coro* (20 sessioni > 30min), *Le Voci di Ilúvatar* (10 sessioni come listener) — calcolati live da `useUserGagliardetti` con collection-group queries Firestore (cache localStorage 1h). *Il Discordante* (5 proposte skippate consecutive) deferito a Fase 2.5: richiede contatore denormalizzato `users/{uid}.consecutiveSkipped`. Catalogo completo + computeGagliardetti in `src/lib/gagliardetti.ts`. UI in `ProfiloPersonale` (card "Gagliardetti" con earned + progress bar per ognuno, raggruppati per categoria).
...
**Batch B7 — Post-audit hardening (Maggio 2026, dopo l'audit B7):**
- **Cap `users.points` increment**: alzato da +50 a +1000 per transazione owner-side. Pre-fix il cap era incoerente col range moltiplicatori 0.5–5.0 (es. `closeSession` con multiplier 5x voleva accreditare 75pt, respinto). Il nuovo cap copre i worst-case legittimi mantenendo il blocco al forging catastrofico.
- **Quiz scoring split owner-side**: `evaluateRoundAnswers` (host-driven, infrangeva la rule `users.update` perché scriveva `users/{altro}.points`) sostituito da `revealRound` (host: pubblica correctIndex + revealedAt) + `claimMyAnswerPoints` (client: ogni partecipante esegue la propria transazione di scoring con localStorage idempotency). Vedi §4.8.
- **Sporche #25/#26 "Theme Hijacker" chiuse al 100%**: `audio_sessions/{}/queue.update` ora ha `affectedKeys.hasOnly([...])` esplicito + immutabilità di `proposedBy`/`localTrackId`/`trackTitle`/`trackArtist`/`trackDurationMs` come defense-in-depth. Cap `pointsAwarded ≤ 50`.
- **`validQueueStatusTransition` stretta**: rimossi gli shortcut `queued → ready`/`queued → failed` non previsti dalla spec; ora il flow segue `queued → transferring → ready/failed → playing/skipped → played` con `failed → skipped` come cleanup path.
- **Cap quiz `pointsAwarded`**: era `pointsMultiplier × 100` (default 100, con multiplier 5 → 500), ora `photoQuizConfig.maxPointsPerRound × pointsMultiplier` (default `100 × 1 = 100`). Nuovo campo `photoQuizConfig.maxPointsPerRound` opzionale (backward-compat con default 100 in rule).
- **Leaderboard ownership-stretto**: `game_events/{}/leaderboard.write` ora richiede `userId == request.auth.uid || isEventOrganizer || isRoot`. Pre-fix qualsiasi participant poteva azzerare il leaderboard di un avversario.
- **Participants delete ownership-stretto**: `game_events/{}/participants.delete` ora ammesso solo a `userId == request.auth.uid || isEventOrganizer || isRoot`.
- **Race-safety `advanceGameEventStatus`**: il branch `→ completed` ora avvolge la write in `runTransaction` con re-check di `status` post-fetch della leaderboard. Due client che terminano l'evento concorrentemente non riscrivono `finalLeaderboard` (uno dei due è no-op).
- **`advanceQuizRound` no-op cleanup**: `status: 'active'` ora scritto solo se diverso da quello corrente.
- **`AuthContext` profile listener leak**: `unsubscribe` ora gestito via `useRef` per detach garantito su transizioni `auth.onAuthStateChanged`.
- **WebRTC signaling timestamp**: `addedAt` ICE candidates e `createdAt`/`expireAt` signaling docs ora usano `Timestamp` Firestore (era `Date.now()` epoch number).
- **PWA icons offline-safe**: sostituite le SVG remote DiceBear con `public/icon.svg` inline (~400 byte). PWA installabile offline al primo run.
- **`scoring.calculateQuizPoints` floor universale**: una risposta corretta entro il tempo è ora sempre ≥ 1pt (era 0 per `9999/10000` nel default `maxPoints=10` per via di `Math.floor(10 × 0.0001) = 0`).
- **Rimosso codice morto**: `setRoundStatus`, `configureQuizRound` orfani da quando `QuizHostCreateRound` scrive direttamente i doc.
- **Test rule estesi**: 6 nuovi `describe` in `firestore.rules.test.ts` (cap 1000, answers self-claim, leaderboard ownership, participants delete, finalLeaderboard immutability) e 1 in `firestore.rules.audio.test.ts` (queue immutable metadata + status transition). `audioEngine.test.ts` riscritto con 8 verifiche concrete (vs 4 smoke `expect(true).toBe(true)`).
...
**Wake Lock feature-policy hardening:** wrappato `navigator.wakeLock.request()` in un hook condiviso `useWakeLock(active)` (`src/hooks/useWakeLock.ts`) che fa feature-detection 2026-style (preferisce `document.permissionsPolicy.allowsFeature('screen-wake-lock')`, fallback a `document.featurePolicy.allowsFeature(...)`, degrada se nessuno dei due è esposto) e re-acquisisce automaticamente il lock su `visibilitychange === 'visible'`. Sostituisce i blocchi inline in `TreasureHuntPlay` e `useAudioPlayer`. Evita warning in iframe (AI Studio preview, Stackblitz, embed di terze parti). Comportamento runtime invariato in PWA installata.
...
**Leaflet markers offline-safe (Maggio 2026):** sostituiti tutti i `new L.Icon({ iconUrl: 'https://...' })` (caricati da `raw.githubusercontent.com`, `cdnjs`, `unpkg`) con `L.divIcon` SVG inline tramite la utility `src/lib/leafletIcons.ts` (`createMarkerIcon('blue'|'gold'|'green'|'crimson')`). PWA offline ora ha i marker; nessun nuovo asset binario in `public/`; bundle delta trascurabile (~600 byte per marker SVG inline).

**Hardening rule audio (Maggio 2026):** nuovo helper rule `effectiveMaxQueued(sessionId)` che riproduce in DSL la formula bonus client-side `getMaxQueuedFor`. La rule `queue.create` ora valida il campo derivato `effectiveMaxAtCreate` snap del valore al doc, chiudendo il vettore di forge della "Sporca #24 Queue Stuffer" (count effettivo dei doc attivi resta CF Fase 2). La rule `signaling/{userId}` è stata spostata da collezione root a sub-collection di `audio_sessions/{sessionId}` con ownership stretto (proposer-or-DJ), chiudendo "Sporca #30 Signaling Spammer".

**DJ scoring runtime (Maggio 2026):** `DJEngine` ora riceve `eventMultiplier: number` in `updateState()` e `getServerTimestamp: () => unknown` in DI. `pointsAwarded` su tracce played = `Math.round(2 × eventMultiplier)` (era hardcoded `2`). `currentTrackStartedAt`, `transferStartedAt`, `transferCompletedAt` sono server-stamped. `closeSession` accredita `+5` base + (se `totalDurationMs > 30 min`) `+10` long-session bonus, scalati × `eventMultiplier` in batch atomico, e marca `djBonusAwarded: true` (immutabile a `true` via rule) per impedire double-spend.

**Hook split `useAudioPlayer` / `useAudioEngineRaw` (Maggio 2026):** API raw del singleton `AudioEngine` (engine, playBlob, pause, resume, stop, getCurrentTime, getDuration, isPlaying) estratte in un hook dedicato `useAudioEngineRaw`, consumato dal pannello DJ. `useAudioPlayer` resta limitato allo state-level del Walkman (queue, shuffle, repeat, MediaSession, wake-lock di background).

**Hook RBAC centralizzato (Maggio 2026):** ~40 occorrenze di `profile?.role === 'X'` migrate al nuovo `src/hooks/useRBAC.ts` che espone `isRoot`/`isAdmin`/`isGuest`/`isAdminOrRoot`/`isApproved`/`isPending`. `useAuth()` resta per i caller che vogliono solo `user`/`profile` raw.

**AuthContext race fixed (Maggio 2026):** `loginWithGoogle()` in `src/lib/firebase.ts` ridotto a thin wrapper su `signInWithPopup`; tutta la creazione del profilo è ora di proprietà esclusiva di `AuthContext.onAuthStateChanged`. Il branch legacy "auto-approva docs senza accountStatus" è ora ristretto ai soli doc con `createdAt < 2024-01-01`; ogni anomalia successiva al cutoff resta `pending` con `console.warn`.

---

## Aggiornamento — round UX 2026-05-06 (post-test utente)

Cinque commit (`c98bb30 → c48ea8e`) di stabilizzazione UX e bug-fix client-side. Riassunto tecnico delle modifiche; il dettaglio per ogni commit è in `STATO_PROGETTO.md`.

### CSS / UI primitives

- `src/index.css` — nuova utility `@layer utilities { .scrollbar-hide { scrollbar-width: none } .scrollbar-hide::-webkit-scrollbar { display: none } }`. Era usata in 9 file ma non definita. Plus restyle globale slim 6px sulle scrollbar di sistema (track trasparente, thumb `#8c928d66` chiaro / `#42a83a59` scuro).

### Schema dati

- `src/types.ts` — `Post.authorPhotoURL?: string | null` (campo nuovo, opzionale, no migration). Salvato da `IlBaule.handleUpload` e `IlBivacco.CreateEventModal` (post type='event'); letto da `LaPiazza` per renderizzare l'`<Avatar>` con la foto reale invece del fallback iniziale.

### Hook GPS — `useHighAccuracyPosition(active=true, highAccuracy=true)`

- Signature: nuovo parametro `highAccuracy` (default `true`, non rompe i caller). One-shot `getCurrentPosition` in parallelo a `watchPosition` per primo fix in 1-2s su desktop (era 5-10s solo watch). Timeout watch `10s → 30s`. Ritorna `error: GeoError | null` invece di `string | null`. Nuova `interface GeoError { code: 1 | 2 | 3; message: string }` esportata. GameCreator passa `highAccuracy=false` (coarse + `maximumAge: 60s`); TreasureHuntPlay resta su `true`.
...
- CTA "Continua senza GPS" → modalità read-only con mappa centrata su `event.treasureHuntConfig.centerLat/Lng` (fallback Marzio `45.9238, 8.8655`); marker visibili, popup "Riattiva il GPS per catturare", `handleOpenAR` mostra alert esplicito.
...
- Step 1 → step 2: pulsante "Avanti" valida `title/description/kickoff` prima di `setStep(2)`; disabled + hint ambra. `kickoff` pre-popolato a `now+10min` in local time (helper `defaultKickoffLocal()`).
- Validation pre-call: `kickoff > now+30s`, `pointsMultiplier ∈ [0.5, 5.0]`. Range guards UI: `radius ∈ [10, 5000]m`, `autoCount ∈ [1, 100]`.
- Default centro mappa Marzio (`45.9238, 8.8655`); era Roma (`41.9028, 12.4964`).
- `hasUserCentered` flag: una volta mosso il centro intenzionalmente, gli update GPS non sovrascrivono.
- Pulsante "Centra su di me" — icona `LocateFixed` (era `Compass`), disabled se `userPosition === null`, tooltip esplicito.
- **Ricerca città/indirizzo** — pulsante `Search` nella top-bar, modal centrata con input + lista risultati Nominatim (`limit=5`, `Accept-Language: it`). Stesso endpoint OSM già usato da `IlBaule`, zero deps. Click su risultato → `setCenter` + `hasUserCentered=true` + chiusura.
- "Salva" step 2 disabled se `items.length === 0`. Guard screen step 2 con CTA "Torna ai Dettagli Evento" se base fields mancano.

### Lobby + setRSVP identity

- `setRSVP(eventId, userId, status, identity?)` — nuovo param opzionale `identity: { displayName?, photoURL? }`, salvato solo nel ramo create (la rule `participants.update` restringe il diff a `['status', 'respondedAt', 'shareLocationDuringEvent', 'leftAt']`).
- `GameEventCard` passa `profile.displayName/photoURL` nelle 4 chiamate setRSVP.
- `GameLobby` usa il componente `<Avatar>` standard al posto del placeholder svg, `p.displayName` invece del fisso "Giocatore" in live + final leaderboard.

### items.create defensivo

- `useGameEvents.createGameItem` — `Math.floor(itemData.points)` + `spawnedAt: serverTimestamp()`.

### geoUtils

- `generateUniformPointsInRadius` — early return `[]` per `count<=0 || radiusMeters<=0`. Clamp `cos(centerLat) >= 0.01` per i poli.

### useAudioQueue.proposeTrackToSession (standalone)

- Idempotently upserta il doc `audio_sessions/{}/participants/{auth.uid}` prima del `setDoc` sulla queue. Pre-fix la rule `queue.create` respingeva con `Missing or insufficient permissions` perché `isSessionParticipant(sessionId)` falliva quando il proposer non aveva mai aperto la sessione come listener.
- `Math.floor(effectiveMaxAtCreate)` per coerenza col rule helper `int(userPoints / 100)`.

### Error handling: surface `err.message` ovunque user-initiated

7 catch generici migrati a `alert(\`...: ${err?.message}\`)` in PhotoQuizPlay, IlBaule (×4: skip-crop, confirm-crop, enhance-AI, address-search, upload), AdminPanel (×2), EventDetailModal (×2). Plus IlBivacco usa `increment(5)` invece di `(user.points||0)+5` (race-safe).

### Componenti UI nuovi (4-5 maggio)

- `src/components/ErrorBoundary.tsx` — class component, fallback CTA, wrappato in `App.tsx` tra `<BrowserRouter>` e `<Suspense>`.
- `src/components/audio/AddToSessionModal.tsx` — modal per "Aggiungi a un Coro" da Library / FullScreenPlayer.
- `FullScreenPlayer` — pill chip "Sfondo" e "EQ" etichettati, bottone X, ListPlus apre AddToSessionModal, prop `onStop`.
- `MiniPlayer` — bottone X (Trash2).
- `TrackCard` — menu "Aggiungi a un Coro" abilitato (era disabled).
- `AudioSessionsList` + `GameEventCard` — bottone Trash2 Root-only.
