...
*Revisione 2026-05-23 — fix di tre regressioni di permessi della stessa classe (validatore `isValidX(incoming())` usato come AND globale su un `allow update` con un check `data.<campo> == request.auth.uid`, che blocca ogni write cross-user legittima):*
- ***Approvazione / cambio-ruolo cross-user respinti.** Il validatore `isValidUser` imponeva `data.uid == request.auth.uid` come AND globale su **ogni** ramo di `users.update`. Quando un Admin/Root scriveva sul doc di un **altro** utente (approvazione `pending → approved`, promozione, cambio ruolo), `incoming().uid` era l'uid del target, non del chiamante: il check identitario falliva prima ancora che il ramo admin/root venisse valutato, **congelando tutti i nuovi iscritti in `pending`** senza modo di abilitarli. Ribasato a `data.uid == userId` (il campo `uid` deve coincidere con l'id del documento) — invariante corretta sia per le self-write (la create già impone `userId == request.auth.uid`) sia per le write admin→altro-utente. Le branch §2 erano corrette: era il guardiano globale a bloccarle.*
- ***"Like Forger" indurito (Sporca #31).** `isValidPost` (che impone `authorId == request.auth.uid`) non è più un AND globale di `posts.update`: un like è una write parziale che lascia `authorId` invariato, quindi `incoming().authorId` restava quello dell'autore e il check **bloccava i "Mi Piace" di chiunque non fosse l'autore** del post. Ora `isValidPost` gating il solo ramo author-edit; i rami like/unlike e comment-bump validano da soli (`affectedKeys().hasOnly` + set-diff pinnato all'uid del chiamante + cap `likedBy ≤ 5000`). Rimosso anche il primo termine ridondante `isAdminOrRoot() || …` che, essendo già garantito dal gate in cima, lasciava a un autore-admin il bypass della logica anti-forge (double-like).*
- ***RSVP agli eventi respinto.** Stesso pattern su `events.update`: `isValidEvent` (che impone `authorId == request.auth.uid`) era AND globale. L'RSVP (`EventDetailModal.handleRSVP`) è una write parziale del solo campo `attendees` che lascia `authorId` invariato → **solo l'autore dell'evento poteva confermare la presenza**; chiunque altro veniva respinto. Ora `isValidEvent` gating il solo ramo author-edit; l'RSVP è un ramo dedicato che pinna la modifica all'**entry propria** della mappa `attendees` (`attendees.diff(...).affectedKeys().hasOnly([request.auth.uid])`, anti "attendee forger").*
- *Nota: `expenses.update` ha lo stesso validatore con `paidBy == request.auth.uid`, ma il client non aggiorna mai una spesa (solo create/read/delete) → footgun dormiente, non corretto per assenza di use-case.*
- *Tutti e tre i fix coperti da regression test in `firestore.rules.test.ts` (describe "Admin/Root user management (cross-user users.update)", "Sporca #31 — Like Forger", "Events update (RSVP cross-user + author edit)").*
...
- **Updates Mappati:** Azioni specifiche, su rami distinti di `posts.update`. **(a) Edit dell'autore:** solo l'autore (`request.auth.uid == existing().authorId`, gated da `isValidPost`) agisce su `caption`, `visibilityStatus`, `visibilityTime`, `showInCinematografo`, o `location`. **(b) "Mi Piace":** qualsiasi utente abilitato (la collezione è admin/root-only) può like/unlike — anche su post altrui, com'è naturale per i cuori — con una doppia validazione tramite array (`likedBy`) che lega matematicamente l'update all'accoppiata di logiche su `likesCount` e **pinna l'elemento aggiunto/rimosso all'uid del chiamante** (set-diff), prevenendo loop, falsificazioni dei conteggi, upvotes infiniti e iniezione dell'uid altrui (Sporca #31). **(c) Comment-bump:** `commentsCount ±1` isolato. Nessuno di questi rami è bypassabile da admin/root: la logica vale per tutti (vedi nota di revisione 2026-05-23).
...
14. **The Teleporter:** Tentativo di catturare un item mentre il giocatore è oltre il `captureRadius`. ✅ **Chiuso Fase 2 (Maggio 2026):** Cloud Function `validateCaptureDistance` (`europe-west1`, `nodejs22`) esegue Haversine server-side e scrive `serverValidatedAt: serverTimestamp()` sul doc item. La rule `items.update` accetta la transizione `spawned → collected` solo se `serverValidatedAt` è stato scritto entro 30s (path 2) oppure se il campo non esiste ancora — fast-path legacy per backward-compat coi vecchi client che non chiamano la CF (`collectedAtLat/Lng` audit log resta come mitigazione su quel branch). `useGameEvents.captureItemTransaction` chiama la CF prima della transazione e fa fallback graceful se la CF non è deployata (`functions/not-found` / `unavailable`).
...
| FCM notifiche pre-evento | feature mancante | Trasversale | ✅ chiuso Fase 2 (Maggio 2026) — `notifyKickoff` CF su `europe-west1`, `users.{uid}.fcmTokens[]` cap 20, Service Worker dedicato, opt-in UI in ProfiloPersonale | — |
...
**Aggiornamento Maggio 2026 (round UX 6 maggio):** una serie di hardening client-side non security-critical ma rilevanti per chiarezza:
- `useAudioQueue.proposeTrackToSession` (chiamata da `AddToSessionModal` su Library/FullScreenPlayer) ora idempotently auto-crea il doc `audio_sessions/{}/participants/{auth.uid}` prima del `setDoc` sulla coda. Pre-fix la rule `queue.create` respingeva con `Missing or insufficient permissions` perché `isSessionParticipant(sessionId)` falliva quando il proposer non aveva mai aperto la sessione come listener. Schema participant identico a quello che `AudioSessionCreate` usa per il DJ. Non apre nuovi vettori: la rule `participants.create` esige già `userId == request.auth.uid` e session aperta.
- `useGameEvents.createGameItem` ora forza `Math.floor(itemData.points)` al boundary, in coerenza con `firestore.rules:330` `points is int`. Difensivo: i template attuali producono interi, ma la rule respingeva silenziosamente eventuali payload float-like.
- `useAudioQueue.proposeTrackToSession` ora applica `Math.floor(effectiveMaxAtCreate)` al boundary, in coerenza con il rule helper `effectiveMaxQueued()` che usa `int(userPoints / 100)`.
- `useHighAccuracyPosition` ora distingue i 3 codici W3C (`PERMISSION_DENIED`, `POSITION_UNAVAILABLE`, `TIMEOUT`) e ritorna `error: { code, message } | null` invece di stringa nuda. `TreasureHuntPlay` applica un grace period di 20s su `TIMEOUT/UNAVAILABLE` prima di mostrare la schermata d'errore (PERMISSION_DENIED resta immediato), e offre un opt-in "Continua senza GPS" che mostra la mappa centrata su `event.treasureHuntConfig.centerLat/Lng` con cattura disabled.
- `setRSVP` ora salva `displayName` e `photoURL` nel ramo create del doc `participants/{}` (la rule update non li ammette nel diff — sono identity field immutabili post-create). GameLobby usa l'`<Avatar>` standard con quei campi.
...
**Estensione R1 — UX round 2026-05-07:** 3 nuovi test rule regression in `firestore.rules.test.ts` describe `12. Game Events Security` documentano il comportamento atteso post-fix client (auto-join organizer come participant; pre-check tempo client):

- **`items.update rejected when caller is not in participants`** — un utente approved che non ha fatto `setRSVP` non può catturare. La precondizione `isEventParticipant(eventId)` resta inviolata. Pre-fix `createGameEvent` non upsertava il participant doc per l'organizer, quindi questo caso si manifestava sempre per chi creava la propria caccia; il client R1 chiama ora `setRSVP(organizerId, 'joined')` automaticamente, ma la rule deve restare a guardia di chiunque (es. utente esterno con un eventId rubato).
- **`items.update rejected before scheduledKickoff (time window)`** — `isWithinTimeWindow(eventId)` richiede `request.time >= scheduledKickoff`. Test conferma che anche un participant joined viene respinto se l'evento è formalmente `active` ma il kickoff schedulato è ancora nel futuro. R1 sul client surface un alert user-friendly (`"L'evento non è ancora ufficialmente iniziato. Aspetta il kickoff alle HH:mm."`) prima di hittare la rule.
- **`items.update succeeds for joined organizer in active window`** — happy-path positive control. Organizer joined come participant + event active + within window + payload con immutabilità rispettata + `serverTimestamp()` su `collectedAt` → cattura accettata.

**Niente nuove Sporche introdotte da R1-R5.** I fix UX sono difensivi/UX-only:
- `useDeviceOrientation` `available` flag è puramente client-side, nessuna superficie nuova esposta lato Firestore.
- Rimozione del `rotate` keyframed in `ARCaptureLayer` è UX-only.
- `.pb-nav-safe` utility e `dvh` sui modali sono CSS.
- `LaMappa` z-index del filtro è z-stack interno.
- `QuizHostCreateRound` `posts.query` ora `limit(50)` — riduce superficie di attacco per memory exhaustion (mitigazione difensiva, non rule change).
- `IlBaule` pre-flight + guest CTA, datetime hint, error message — UX/a11y.

I 3 nuovi test sono **regression locks**: documentano che l'auto-join organizer client-side e il kickoff guard server-side devono restare entrambi. Se in futuro il client smettesse di fare l'auto-join (es. refactor di `createGameEvent`), il primo test rule fallirebbe al CI, segnalando il regression prima che l'organizer ne accusasse il sintomo in produzione.
